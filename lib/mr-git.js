...
class MrGit {
  ...
  addChangeListener() {
    ...
    this.repo.onDidChangeStatus(function(evt) {
      ...
      var item = self.container.querySelector(`[data-file-path="${evt.path}"]`);
      console.log("items...", item);

      //Todo, path and status is already defined, no need to get them again
      if (item) {
        var path = evt.path
        var status = evt.pathStatus;
        self.colorItems(status, item);
      }
    });
  }
